//===- CS515.h - Header for CS 515 projects ----------------------------*- C++ -*-===//
//
...
//===----------------------------------------------------------------------===//
//
// This file provides the interface for the passes of CS 515 projects
//
//===----------------------------------------------------------------------===//

#ifndef LLVM_TRANSFORMS_515DFA_H
#define LLVM_TRANSFORMS_515DFA_H
...
//   protected:
  ...
  public:
	...
	// const std::map<unsigned int, Instruction *> getIndexToInstr() {
    //   return IndexToInstr;
    // }

    std::map<Instruction *, unsigned> getInstrToIndex() const {
      return InstrToIndex;
    }

    std::map<Edge, Info *> getEdgeToInfo() const {
      return EdgeToInfo;
    }
        ...
        while (!worklist.empty()) {
            ...
            for(unsigned i=0; i<info_out.size(); i++) {
                ...
                if(!Info::equals(newInfo, EdgeToInfo[edge])){
					// errs()<<"writing to EdgeToInfo..."<<"\n";
                    // if (EdgeToInfo[edge] != &Bottom) { 
                    //     delete EdgeToInfo[edge];
                    // }
					// errs()<<"updating "<<start<<"->"<<end<<"\n";
					// errs()<<"worklist joining...\n";
					// info_out[i]->print();
					// EdgeToInfo[edge]->print();
					// errs()<<"end of worklist join...\n";

                    EdgeToInfo[edge] = newInfo;
                    worklist.push_back(end);

					// errs()<<"pushing node "<<end<<" to the worklist...\n";
                } else {
                    delete newInfo;
                }
            }
